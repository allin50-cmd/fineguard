#!/usr/bin/env node
/**
 * Final Working Import - Prepared Statements with Safe Batch Size
 * PostgreSQL limit: 65535 params. With 24 columns: max 2730 rows/batch
 * Using 1000 rows/batch = 24,000 params (safe margin)
 */
...
const BATCH_SIZE = 1000; // 1000 rows × 24 columns = 24,000 params (well under 65K limit)
...
async function insertBatch(client, records) {
  ...
  for (const record of records) {
    ...
    params.push(
      ...
      record.reg_address_care_of,
      record.reg_address_po_box,
      record.reg_address_line1,
      record.reg_address_line2,
      record.reg_address_post_town,
      record.reg_address_county,
      record.reg_address_country,
      record.reg_address_postcode,
...
async function importCompanies() {
  console.log('🚀 Final import - 5.8M companies with enrichment fields\n');
  console.log('📁 Source:', CSV_FILE);
  console.log('📦 Batch size:', BATCH_SIZE, 'rows (24,000 params)');
  console.log('⚡ Updates every 100k records\n');
  ...
  try {
    ...
    // Check current count - DON'T truncate if data exists
    ...
    if (currentCount > 100000) {
      console.log(`⚠️  Database already has ${currentCount.toLocaleString()} companies!`);
      console.log('Skipping import to preserve existing data.');
      console.log('If you want to re-import, manually run: TRUNCATE TABLE companies CASCADE;');
      process.exit(0);
    }

    if (currentCount > 0) {
      console.log(`Found ${currentCount} existing records. Truncating...`);
    ...
    stream.pause(); // Pause to control flow
    ...
    for await (const row of stream) {
      ...
      if (batch.length >= BATCH_SIZE) {
        ...
        if (totalInserted % 100000 === 0) {
          ...
          console.log(`   📊 ${totalInserted.toLocaleString()} (${rate}/sec, ~${remaining}min left)`);
        }
      }
    }
    ...
    console.log(`\n✅ Import complete!`);
    console.log(`   Records: ${totalInserted.toLocaleString()}`);
    console.log(`   Time: ${Math.round(totalTime / 60)}min ${Math.round(totalTime % 60)}sec\n`);
    ...
    console.log('📊 Final: Total', parseInt(stats.rows[0].total).toLocaleString(),
                '| Active', parseInt(stats.rows[0].active).toLocaleString());

    console.log(`\n⏱️  Total: ${Math.round((Date.now() - startTime) / 60000)}min\n`);
    console.log('🎉 Success!\n');
